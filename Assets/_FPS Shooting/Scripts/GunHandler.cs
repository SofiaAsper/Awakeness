...
public class GunHandler : MonoBehaviour
{
    ...
    public float MeleeHitTime = 0.8f;
    ...
    private void Start()
    {
        Transform Spawn = bulletSpawn.transform.parent;
        for (int i = 0; i < Spawn.childCount; i++)
        {
            if (Spawn.GetChild(i).GetComponent<MeleeDamage>()!=null)
            {
                MeleeSphere = Spawn.GetChild(i).gameObject;
                Spawn.GetChild(i).GetComponent<MeleeDamage>().damageAmount = gun.bulletDamage;
            }
        }
    }
    private void OnEnable()
    {
        ...
        {
            for (int i = 0; i < gunController.gunInventory.Count; i++)
            {
                if (gun.GetHashCode() == gunController.gunInventory[i].gun.GetHashCode())
                {
                    gunIndex = i;
                    break;
                }
                else gunIndex = gunController.gunInventory.Count + 2;
            }
        }
    }
    ...
    private void Update()
    {
        transform.localRotation = Quaternion.Slerp(transform.localRotation,
            (isAiming()) ? Quaternion.Euler(gun.ironSightAim.eulerAngles) : Quaternion.identity,4f);
    ...
    public bool ShootGun()
    {
        ...
        adjustSpeed = (isAiming()) ? 12f : Random.Range(10.0f, 12.0f);
