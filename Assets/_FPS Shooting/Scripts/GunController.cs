...
public class GunController : MonoBehaviour
{
    ...
    public int maxGunsInInventory = 2;

    public bool toggleADS = false;
    [Range(1f, 2f)]
    public float actionADSMultiplier = 1.25f;
    [Range(0f, 1f)]
    public float crouchADSMultiplier = 0.55f;
    ...
    void ResetGuns()
    {
        ...
        if (gunSelected())
        {
            ...
            if (ui)
            {
                ui.UpdateSelectedGunUI(SelectedGun());
                ui.UpdateNextPrevGuns(gunInventory[prevGun], gunInventory[nextGun]);
            }
        }
    }
    ...
    {
        //Debug.Log("Selected gun : " + selectedGun + " : Inventory :"+ gunInventory.Count);
        return gunInventory[selectedGun];
    }
    public GunHandler PreviousGun()
    {
        return gunInventory[prevGun];
    }
    public GunHandler NextGun()
    {
        return gunInventory[nextGun];
    }
    //************************************************************************************************
    private void Update()
    {
    ...
    void SpreadHandler()
    {
        if (gunHandler)
        {
            ...
            if (isAiming())
            {
                ...
                actualSpread = Mathf.Lerp(actualSpread, spread, Time.deltaTime * 3f);
...
// if aim button on controller is pressed then the delay will be 0
    // 
        /// managing delay timer /// update for mobile ########################################################################
    ...
    public bool GetGunAim()
    {
        if (gunHandler.gun.AimStop)
        {
            return true;
        }
        else
    ...
    public float GetDelayShootOnMobile()
    {
        return gunHandler.gun.fireDelayOnFreeShoot;
    }
    ...
    void AimingHandler()
    {
        if (gunHandler)
        {
            ...
            if ((int)status < 2)
            {
                ...
                {
                    if (!toggleADS)
                        status = (input.aim) ? GunControlStatus.aiming : GunControlStatus.ready;
                    if (!input.aim)
    ...
    void SwitchWeaponHandler()
    {
        ...
        if (gunSelected() && input.toChange)
        {
            ...
            {
                float gunSwitch = input.gunSwitch;
                if (gunSwitch >= 0.5f)
                {
                    ChangeSelectedGun(1);
                    input.toChange = false;
                }
                else if (gunSwitch <= -0.5f)
                {
                    ChangeSelectedGun(-1);
                    input.toChange = false;
                }
    ...
    void ChangeSelectedGun(int add)
    {
        ...
        else if (selectedGun == gunInventory.Count - 1)
        {
            prevGun = selectedGun - 1;
            nextGun = 0;
        }
    ...
    public void TakeOutSelectedGun()
    {
        ...
        if (ui)
        {
            ui.UpdateSelectedGunUI(gunHandler);
            ui.UpdateNextPrevGuns(gunInventory[prevGun], gunInventory[nextGun]);
        }
    ...
    {
        canHit = false;
        yield return new WaitForSeconds(gunHandler.MeleeHitTime);
        canHit = true;
    }
    void FireGun()
    {
        if (gunHandler.isMelee)
        {
            if (canHit == true)
            {
            ...
            if (leftTurn == false)
            {
                gunHandler.gameObject.GetComponent<Animator>().Play(gunHandler.motions.hipfireShoot);
                leftTurn = true;
            }
            else
            {
                gunHandler.gameObject.GetComponent<Animator>().Play(gunHandler.motions.aimedShoot);
                leftTurn = false;
            }
            return;
        }
    ...
    {
        if (gunInventory[index].gun.startingClips < 0)
            return; //We won't reload a gun that cannot reload
    ...
    void RaycastShot()
    {
        ...
        if (Physics.Raycast(shotRay, out var hit, gun.bulletRange, collisionLayer))
        {
            ...
            {
                enemy = damaged.transform.GetComponentInParent<TargetDamageable>();
                if (!damaged.DamageableAlreadyDead())
                {
                    enemy.SetHitPos(impactPos);
    ...
    public void RefillAmmo()
    {
        foreach (GunHandler handler in gunInventory)
        {
            ...
            if (gun.startingClips >= 0 && (handler.totalAmmo < gun.ammoClip * gun.startingClips || handler.ammoInClip < gun.ammoClip))
            {
    ...
    public void AddGunTemporarily(GunObject addGun) //Will not change the prefab, this should be called if the game is running
    {
        RemoveBlanks();
        putAwayGun = selectedGun;
        //i have comented this because it was removing gun and not giving the right index
        //RemoveGunTemp(putAwayGun);
        ...
        if (handler == null) //If we did not find a handler with the gun using the gun name
        {
            ...
            gunInventory.Add(handler);
            //i have comented this because it was not giving right index
            //handler.gunIndex = maxGunsInInventory - 1;
            handler.gunIndex = gunInventory.Count-1;
        }
        else
        {
        ...
        if (addGun.shootType.ToString() == "Melee")
        {
            handler.isMelee = true;
        }
        //Swap to new gun
    ...
    //****************************************************
    public void RemoveGunTemp(int removeGunIndex)
    {
        GunHandler removedGun = gunInventory[removeGunIndex];
        if (gunInventory.Count == maxGunsInInventory)
        //remove active gun
        {
            if (removedGun.gun != null)
            {

                GameObject gunPrefab = removedGun.gun.gunPickupObj;
                Vector3 gunWorldPos = new Vector3(transform.position.x, .5f, transform.position.z);
                Instantiate(gunPrefab, gunWorldPos, gunPrefab.transform.rotation);
                gunInventory.RemoveAt(removeGunIndex);
                if (removeGunIndex != maxGunsInInventory - 1)
                {
                    for (int i = removeGunIndex; i < gunInventory.Count; i++)
                        gunInventory[i].gunIndex--;
                }
                removedGun.gunIndex = maxGunsInInventory + 2;
                removedGun.gameObject.SetActive(false);
            }
        }
    }
    ...
    {
        if (handler.gun.shootType.ToString() == ("Melee"))
        {
            GameObject newMelee = GameObject.Instantiate(handler.gun.prefabObj);
            newMelee.name = handler.gun.prefabObj.name;
            newMelee.transform.SetParent(handler.transform);
            TransformHelper.SetLocalTransformData(newMelee.transform, handler.gun.prefabLocalData);
        }
        else
        {
